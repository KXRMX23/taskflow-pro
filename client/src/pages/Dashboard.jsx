...
function Dashboard() {
  ...
  // Reusable card — eliminates the triplicated JSX from before
    ...
    <Draggable key={task.id} draggableId={task.id.toString()} index={index}>
      {(provided) => (
        ...
        >
          ...
          <h3 className={`text-3xl font-bold tracking-tight mb-3 transition-all duration-300 group-hover:translate-x-1 ${
            darkMode ? "text-white" : "text-gray-900"
          }`}>
            {task.title}
          </h3>

          <p className={`mb-5 leading-7 ${darkMode ? "text-gray-300" : "text-gray-600"}`}>
            {task.description}
          </p>
        ...
        <div className="max-w-6xl mx-auto px-6 pt-12 pb-10">
          ...
          <div className="flex items-center justify-between mb-10">
            <div className="flex items-center gap-5">
              ...
              <div>
                <h1 className="text-5xl font-extrabold tracking-tight">TaskFlow Pro</h1>
                <p className="text-lg text-gray-500 mt-1">Organize your work efficiently</p>
              </div>
            </div>
            <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            <motion.div
              ...
              transition={{ duration: 0.5, delay: 0 }} whileHover={{ scale: 1.03 }}
            ...
            >
              ...
              <p className="text-5xl font-extrabold mt-2 group-hover:scale-110 transition-transform duration-300">{pendingCount}</p>
              <p className="text-sm text-gray-600 mt-2">Tasks waiting to be started</p>
            </motion.div>

            <motion.div
              initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5, delay: 0.15 }} whileHover={{ scale: 1.03 }}
            ...
            >
              ...
              <p className="text-5xl font-extrabold mt-2 group-hover:scale-110 transition-transform duration-300">{inProgressCount}</p>
              <p className="text-sm text-gray-600 mt-2">Tasks currently being worked on</p>
            </motion.div>

            <motion.div
              initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5, delay: 0.3 }} whileHover={{ scale: 1.03 }}
            ...
            >
              ...
              <p className="text-5xl font-extrabold mt-2 group-hover:scale-110 transition-transform duration-300">{completedCount}</p>
          ...
          {/*
            FIXES APPLIED HERE:
            1. className was using single quotes ' ' — template literal ${} never evaluated.
               Fixed to backticks ` `.
            2. The opening <form> tag was missing its closing >, breaking all JSX below it.
          */}
          <motion.form
            initial={{ opacity:0, y: 30 }}
            animate={{ opacity:1, y: 0  }}
          ...
          >
            ...
            <textarea
              ...
              onChange={handleChange}
              className={`w-full border rounded-lg p-3 mb-4 transition-all duration-300 ${
                darkMode ? "bg-gray-700 text-white border-gray-600 placeholder-gray-400" : "bg-white text-black border-gray-300"
              }`}
            />
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  <Droppable droppableId="in-progress">
                    {(provided) => (
                      <div ref={provided.innerRef} {...provided.droppableProps}>
                        {inProgressTasks.map((task, index) => <TaskCard key={task.id} task={task} index={index} />)}
                        {provided.placeholder}
                      </div>
                    )}
                  </Droppable>
